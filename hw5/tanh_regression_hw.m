...
function [b,w,c,v,obj_val] = tanh_grad_descent(x,y,i)
    ...
    while k <= max_its        
        % update gradients 
        vmx = repmat(v',P,1); %Px4
        cmx = repmat(c',P,1); %PX4
        wmx = repmat(w',P,1); %PX4
        q_innersum = sum(tanh(cmx+x.*vmx).*wmx,2); %Px1
        q = b + q_innersum - y; %PX1
        grad_b = 2*l_P'*q; 
        
        dw1 = 2*l_P'*(q.*tanh(c(1)*l_P+x.*v(1)));
        dw2 = 2*l_P'*(q.*tanh(c(2)*l_P+x.*v(2)));
        dw3 = 2*l_P'*(q.*tanh(c(3)*l_P+x.*v(3)));
        dw4 = 2*l_P'*(q.*tanh(c(4)*l_P+x.*v(4)));
        grad_w = [dw1; dw2; dw3; dw4];
        
        dc1 = 2*l_P'*(q.*sech(c(1)*l_P+x.*v(1)).^2)*w(1);
        dc2 = 2*l_P'*(q.*sech(c(2)*l_P+x.*v(2)).^2)*w(2);
        dc3 = 2*l_P'*(q.*sech(c(3)*l_P+x.*v(3)).^2)*w(3);
        dc4 = 2*l_P'*(q.*sech(c(4)*l_P+x.*v(4)).^2)*w(4);
        grad_c = [dc1; dc2; dc3; dc4];
        
        dv1 = 2*l_P'*(q.*sech(c(1)*l_P+x.*v(1)).^2.*x)*w(1);
        dv2 = 2*l_P'*(q.*sech(c(2)*l_P+x.*v(2)).^2.*x)*w(2);
        dv3 = 2*l_P'*(q.*sech(c(3)*l_P+x.*v(3)).^2.*x)*w(3);
        dv4 = 2*l_P'*(q.*sech(c(4)*l_P+x.*v(4)).^2.*x)*w(4);
        grad_v = [dv1; dv2; dv3; dv4];
        
        % take gradient steps 
        b = b - alpha*grad_b;
        w = w - alpha*grad_w; %4x1
        c = c - alpha*grad_c;   
        v = v - alpha*grad_v;
